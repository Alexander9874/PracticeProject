    ...
    for i in _zipped_folders_labels_images(training_dir_path, labels):
        for fileName in i[2]:
            ...
            if len(imgEncoding) > 1:
                print('\x1b[0;0;43m' + 'More than one face found in {}. Only considering the first face.'.format(file_path) + '\x1b[0m')
            if len(imgEncoding) == 0:
                print('\x1b[0;0;41m' + 'No face found in {}. Ignoring file.'.format(file_path) + '\x1b[0m')
            else:
...
print('\x1b[6;0;42m' + "Saving labels pickle to'{}'".format(labels_fName) + '\x1b[0m')
with open(labels_fName, 'wb') as f:
    pickle.dump(le, f)
print('\x1b[6;0;42m' + "Training Image's encodings saved in {}".format(encoding_file_path) + '\x1b[0m')
